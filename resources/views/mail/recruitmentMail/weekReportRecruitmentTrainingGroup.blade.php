<table style="width:100%;border:1px solid #231f20;border-collapse:collapse;padding:3px">
    ...
    <tr>
        <td colspan="11" style="border:1px solid #231f20;text-align:left;padding:6px;background:#231f20">Raport od {{$date_start}} do {{$date_stop}}</td>
    </tr>
    <tr>
    ...
    @foreach($data as  $item)
        @php
            $stat1 = $item->countHireUserFromFirstTrainingGroup > 0 ? 100 * $item->sum_choise_stageOne / $item->countHireUserFromFirstTrainingGroup : 0;
    ...
    <tr>
        @php
            $stat_total_1 = $data->sum('countHireUserFromFirstTrainingGroup') > 0 ? 100 * $data->sum('sum_choise_stageOne') / $data->sum('countHireUserFromFirstTrainingGroup') : 0;
            $stat_total_2 = $data->sum('sum_choise_stageTwo') > 0 ? 100 * $data->sum('sum_choise_stageOne') / $data->sum('sum_choise_stageTwo') : 0;
            $stat_total_3 = $data->sum('sum_choise_stageOne') > 0 ? 100 * ($data->sum('sum_choise_stageOne') + $data->sum('sum_absent_stageOne')) / $data->sum('sum_choise_stageOne') : 0;
        @endphp
