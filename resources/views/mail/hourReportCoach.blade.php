<table style="width:100%;border:1px solid #231f20;border-collapse:collapse;padding:3px">
    ...
    <tbody>
        ...
        @foreach($data as $coach)
            ...
            @foreach($coach as $item)
                @if(is_object($item))
                    ...
                    $real_time = round((strtotime($item->stop_time) - strtotime($item->start_time))/3600, 2);
                    $avg = ($real_time > 0) ? round($item->success / $real_time ,2) : 0;
                    @endphp

                    <tr>
                        <td style="border:1px solid #231f20;text-align:center;padding:3px">{{ $item->user_last_name . ' ' . $item->user_first_name }}</td>
                        <td style="border:1px solid #231f20;text-align:center;padding:3px">{{ $avg }}</td>
                        ...
                        <td style="border:1px solid #231f20;text-align:center;padding:3px">{{ $real_time }}</td>
                    </tr>
                    @php
                        ...
                        $total_login_time += round((strtotime($item->stop_time) - strtotime($item->start_time))/3600, 2);
                        ...
                        $hours_array = explode(':', $item->login_time);
                        $day_total_login_time += round((strtotime($item->stop_time) - strtotime($item->start_time))/3600, 2);
            ...
            <tr>
                ...
                <td style="background-color: #efef7f;border:1px solid #231f20;text-align:center;padding:3px"><b>{{ $total_time }}</b></td>
            </tr>
        @endforeach

        @php
            $day_total_time = $day_total_login_time;
        ...
        <tr>
            ...
            <td style="background-color: #c67979;border:1px solid #231f20;text-align:center;padding:3px">{{ $day_total_time }}</td>
        </tr>
    </tbody>
</table>
