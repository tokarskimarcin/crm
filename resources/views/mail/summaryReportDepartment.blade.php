@foreach($data as $dep)
    @php
        ...
        $month = $dep['month'];
        $year = $dep['year'];
        $hour_reports = $dep['hour_reports'];
        $dep_info = $dep['dep_info'];
        $schedule_data = $dep['schedule_data'];
        $months = $dep['months'];
        $dep_data = $departments->where('id', '=', $dep_info->id)->first();
        $dep_name = $dep_data->departments->name . ' ' . $dep_data->department_type->name;
    @endphp
    <table style="width:100%;border:1px solid #231f20;border-collapse:collapse;padding:3px">
        ...
        <tr>
            <td colspan="15" style="border:1px solid #231f20;text-align:center;padding:3px;background:#231f20;color:#efd88f">
                <font size="6" face="Calibri">RAPORT MIESIĘCZNY {{mb_strtoupper ($dep_name, "UTF-8")}} {{$date_start . ' - ' . $date_stop}} </font></td>
        ...
        @for($i = 1; $i <= $total_days; $i++)
            ...
            if ($add_default_zero == false) {
                //Sprawdzenie czy dzien jest noralny czy weekendowy
                ...
                //Pobranie celu RBH
                $working_hours_goal = round($goal / $dep_info->commission_avg);
                //Obliczenie % celu
            ...
            */
            ...
            @if($add_week_total == true || $i == $total_days)
                ...
                <tr>
                    ...
                    <td style="background-color: #c67979;border:1px solid #231f20;text-align:center;padding:3px"><b>{{($total_week_checked != null && $total_week_checked > 0) ? round((($total_week_bad* 100) / $total_week_checked) , 2) : 0 }} %</b></td>
