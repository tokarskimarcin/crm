...
//        dd($reportIds);
    ...
    public function pageMonthReportCoachPost(Request $request) {
        ...
        if(strtotime($date_start) < strtotime(StatisticsGlobalVariables::$firstNovember)){
            $onlyUserID = $this::getNewUsers('<')->pluck('id_user')->toArray();
        }else{
            $onlyUserID = NewUsersRbhReport::RBHUsersBetweenDates([$date_start, $date_stop])->get()->pluck('user_id')->toArray();
            ...
            if($request->onlyNewUser == 1){
                ...
                foreach ($monthData as $userId => $userData){
                    if(in_array($userId, $RBHUsersBetweenDates)){
                        $monthData->pull($userId);
                    }
                }
            }*/
        }

        //dd($monthData, $onlyUserID);
        return view('reportpage.MonthReportCoach')
            ->with([
                'coaches'           => $coaches,
                'date_start'        => $date_start,
 ...
 // RANKING
    private function getWeekMonthCoachDataRanking($date_start, $date_stop, $coach_id) {
        ...
        if($date_start >= StatisticsGlobalVariables::$firstJune) {
            $max_from_day = $max_from_day
                ->where('pbx_report_extension.pbx_department_info', '=', $pbx_department_id);
            if($date_start >= StatisticsGlobalVariables::$firstAugust){
        ...
        }else if($date_start < StatisticsGlobalVariables::$firstJune){
...
//                    $week_yanky += ($report->success * ($report->dkj_proc / 100));
    ...
    public function pageMonthReportCoachSummaryPost(Request $request) {
        ...
        $newUserID = [];
        if(strtotime($data_start) < strtotime(StatisticsGlobalVariables::$firstNovember)){
                $newUserID = $this::getNewUsers('<')->pluck('id_user')->toArray();
        }
        else{
            $newUserID =  NewUsersRbhReport::RBHUsersBetweenDates([$data_start,$data_stop])->get()->pluck('user_id')->toArray();
            ...
            if($request->onlyNewUser == 1){
                ...
                foreach ($data as $coachId => $coach){
                    foreach ($coach['trainer_data'] as $dataKey => $trainer_data){
                        foreach ($trainer_data[0] as $key => $user){
                            if(in_array($user['user_id'], $RBHUsersBetweenDates)){
                                $data[$coachId]['trainer_data']->pull($dataKey);
                            }
                        }
                    }
                }
    ...
    function getNewUsers($comparator){
        $iTimeInSHours = UsersGlobalVariables::$newUsersRbh;
        $CusersWorkingLessThanNewUsersRbh = Work_Hour::usersWorkingRBHSelector($iTimeInSHours, $comparator)->unique('id_user');
        return $CusersWorkingLessThanNewUsersRbh;
    }
    ...
    public function pageDayReportCoachPost(Request $request) {
        ...
        $data = DB::table('pbx_report_extension')
            ...
            '));
            if($request->day_select < StatisticsGlobalVariables::$firstJune) {
            ...
            if($request->day_select > StatisticsGlobalVariables::$firstAugust){
        ...
        if(strtotime($request->day_select) < strtotime(StatisticsGlobalVariables::$firstNovember)){
            if($request->onlyNewUser == 1){
                $data = $data->whereIn('user_id',$this::getNewUsers('<')->pluck('id_user'));
            }else if($request->onlyNewUser == 2){
                $data = $data->whereIn('user_id',$this::getNewUsers('>=')->pluck('id_user'));
            }
        }else{
            ...
            if($request->onlyNewUser == 1){
                ...
                $data = $data->whereNotIn('user_id',$RBHUsersBetweenDates);
            }
        }
    ...
    public function pageSummaryDayReportCoachesPost(Request $request) {
        ...
        if (strtotime($request->day_select) < strtotime(StatisticsGlobalVariables::$firstNovember)) {
            ...
            if ($request->onlyNewUser > 0) {
                $onlyUserID = NewUsersRbhReport::RBHUsersBetweenDates([$request->day_select, $request->day_select])->get()->pluck('user_id')->toArray();
                /*if ($request->onlyNewUser == 1) {
                    $onlyUserID = $RBHUsersBetweenDates;
                } else if ($request->onlyNewUser == 2) {
                    foreach ($data['data'] as $coach) {
                        foreach ($coach as $key => $item) {
                            if (is_object($item)) {
                                if (in_array($item->user_id, $RBHUsersBetweenDates)) {
                                    $coach->pull($key);
                                }
                            }
                        }
    ...
    public function monthReportConsultantPost(Request $request) {
        ...
        if(strtotime($date_start) < strtotime(StatisticsGlobalVariables::$firstNovember)){
            ...
            $newUserID = NewUsersRbhReport::RBHUsersBetweenDates([$date_start,$date_stop])->get()->pluck('user_id')->toArray();
            /*$RBHUsersBetweenDates = NewUsersRbhReport::RBHUsersBetweenDates([$request->day_select,$request->day_select])->get()->pluck('user_id')->toArray();
            if($request->onlyNewUser == 1){
                $newUserID =$RBHUsersBetweenDates;
            }else if($request->onlyNewUser == 2){
                foreach ($data as $key => $dataInfo){
                    if(in_array($dataInfo['consultant']->id, $RBHUsersBetweenDates)){
                        $data->pull($key);
                    }
                }
            }*/
...
//            $users = User::whereIn('id', [6009, 1364])->get();
    ...
    public static function getDepartmentsAveragesForEveryHour($date, $departments_info){
        $reportData = HourReport::where('report_date', '=', $date)->get();
    ...
    private function recruitmentRotationVariables($view, $date_start, $date_stop, $department){
        $rbh = UsersGlobalVariables::$newUsersRbs; //40RBH in seconds
        ...
        $data = $departmentStats->map(function ($item, $key) use ($departmentStats2, $departmentStats3) {
            ...
            $users_less_new_users_sum = $departmentStats3->where('id', $item->id)->first();
            return (object)array_merge($item->toArray(),$working_user_sum->toArray(), $users_less_new_users_sum->toArray());
        });
