...
//        dd($weekDateArr);
    ...
    private function provisionSystemForCoordinators(&$user, $dividedMonth, $month, $year) {
        ...
        $firstDayOfMonthDateTime = new DateTime(date('Y-m-d', strtotime($year .'-'. $month .'-01')));
...
//        dd($weekDateArr);
        ...
        foreach($weekDateArr as $weekInfo) {
            ...
            $campaignData = $this->getCampaignData('2018-08-01', '2018-08-12', 1);
            $databasePercentageUsage = $campaignData = 100 * $campaignData[0]->sum_active / $campaignData[0]->sum_campaign; //percent of database usage

            $data = $this->getMultiDepartmentData('2018-08-01', '2018-08-31', $month, $year, $departments->pluck('id')->toArray(), $days_in_month);
            ...
            $lastDayOfWeekDateTime = new DateTime('2018-08-12');
            $dateDiff = $lastDayOfWeekDateTime->diff($firstDayOfMonthDateTime)->days;
            for($i = 1; $i <= intval($dateDiff); $i++) {
                $day = ($i < 10) ? '0' . $i : $i;
                $date = $year . '-' . $month . '-' . $day;
...
//            dd($total_week_goal_proc);
            array_push($goals, $total_week_goal_proc);
            array_push($provisions, ProvisionLevels::get($databasePercentageUsage, 'koordynator', $total_week_goal_proc, $daysInPosition));
        }
        dd($goals);
    }
